...
macro_rules! test {
    (@base $( #[$attr:meta] ),*$func:ident, $input:expr, $output:expr, $options:expr) => {
        ...
        fn $func() {
            let sass = scss_rust::from_string($input.to_string(), &$options)
    ...
    ($( #[$attr:meta] ),*$func:ident, $input:expr, $output:expr) => {
        test!(@base $(#[$attr])* $func, $input, $output, scss_rust::Options::default());
    };
}
...
macro_rules! error {
    (@base $( #[$attr:meta] ),*$func:ident, $input:expr, $err:expr, $options:expr) => {
        ...
        #[allow(non_snake_case)]
        fn $func() {
            match scss_rust::from_string($input.to_string(), &$options) {
    ...
    ($( #[$attr:meta] ),*$func:ident, $input:expr, $err:expr) => {
        error!(@base $(#[$attr])* $func, $input, $err, scss_rust::Options::default());
    };
...
macro_rules! assert_err {
    ($err:literal, $input:expr) => {
        match scss_rust::from_string($input.to_string(), &scss_rust::Options::default()) {
            Ok(..) => panic!("did not fail"),
            Err(e) => assert_eq!(
                $err,
                e.to_string()
